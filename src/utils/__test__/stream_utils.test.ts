...
describe('handleStreamResponse', () => {
  ...
  it('should process answer chunks', async () => {
    const mockResponse = {
      data: [Buffer.from('data: {"answer": "Hello"}\n'), Buffer.from('data: {"answer": " World"}\n')],
    };
    const onData = vi.fn();
  ...
  it('should handle invalid JSON gracefully', async () => {
    const consoleSpy = vi.spyOn(logger, 'warn').mockImplementation(() => logger);
    const mockResponse = {
    ...
    expect(result).toBeUndefined();
    expect(onData).not.toHaveBeenCalled();
    expect(consoleSpy).toHaveBeenCalledWith('Failed to parse streaming data:', expect.any(Error));
  });

  it('should ignore non-data lines', async () => {
    const mockResponse = {
      data: [Buffer.from('ignore: {"answer": "Ignore this"}\ndata: {"answer": "Hello"}\n')],
    };
    const onData = vi.fn();
