...
const GeocodeFeatureSchema = z.object({
  ...
  properties: z.object({
    ...
    accuracy: z.string().optional().describe('Accuracy of the geocoding result'),
  }),
});
...
const GetCoordinatesToolSchema = {
  ...
  description: 'Convert a location name or address into geographic coordinates using Mapbox Geocoding API',
  parameters: z.object({
    location: z.string().describe('Location name or address to geocode'),
    country: z.string().optional().describe('Limit results to a specific country (ISO 3166-1 alpha-2 code)'),
    type: z
      .enum([
        'country',
        'region',
    ...
    limit: z.number().min(1).max(10).default(1).describe('Number of results to return'),
  }),
...
const GetLocationFromCoordinatesToolSchema = {
  ...
  description: 'Convert geographic coordinates into a location name or address using Mapbox Reverse Geocoding API',
  parameters: z.object({
...
const GetDirectionsToolSchema = {
  ...
  parameters: z.object({
    origin: z.string().describe("Starting location (address or 'longitude,latitude' coordinates)"),
    destination: z.string().describe("Destination location (address or 'longitude,latitude' coordinates)"),
    profile: z.enum(['driving', 'walking', 'cycling']).default('driving').describe('Mode of transportation'),
    alternatives: z.boolean().default(false).describe('Whether to return alternative routes'),
    avoid: z.array(z.string()).optional().describe("Features to avoid (e.g., ['toll', 'motorway'])"),
  }),
...
export class MapboxTool extends APITool<GeocodeParams | DirectionsParams | ReverseGeocodeParams> {
  schema = [
    {
  ...
  constructor() {
    super({
      name: 'Mapbox',
      description: 'Convert locations to coordinates, coordinates to locations, and get directions between places',
  ...
  async geocodeLocation(location: string): Promise<[number, number]> {
    try {
      ...
      const response = await fetch(`${MAPBOX_GEOCODING_URL}/forward?${params.toString()}`);
      if (!response.ok) {
  ...
  async getReverseGeocodingData(params: ReverseGeocodeParams): Promise<z.infer<typeof GeocodeResponseSchema>> {
    try {
      const validatedParams = GetLocationFromCoordinatesToolSchema.parameters.parse(params);
      ...
      const response = await fetch(`${MAPBOX_GEOCODING_URL}/reverse?${queryParams.toString()}`);
      ...
      if (error.name === 'ZodError') {
        throw new Error(`Invalid parameters or response format: ${error.message}`);
      }
  ...
  async getDirections(params: {
    ...
    try {
      const coordString = params.coordinates.map(coord => coord.join(',')).join(';');
  ...
  async getGeocodingData(params: GeocodeParams): Promise<z.infer<typeof GeocodeResponseSchema>> {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/forward?${queryParams.toString()}`);
