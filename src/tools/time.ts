...
const TimestampUnitSchema = z
  .enum(["ms", "sec"])
  .describe("Unit of the timestamp (milliseconds or seconds)");

const TimestampConverterToolSchema = {
  name: "convert_timestamp",
  description:
    "Converts timestamps between ISO strings and numeric values (milliseconds or seconds)",
  parameters: z.object({
    value: z
      .union([z.string(), z.number()])
      .describe("Timestamp value to convert (ISO string or numeric)"),
...
export class TimestampConverterTool extends APITool<{
  ...
  private convertFromISO(
    isoString: string,
    targetUnit: "ms" | "sec" | undefined
  ): string {
    if (!targetUnit) {
      throw new Error(
        "Target unit is required when converting from ISO string"
      );
    }
