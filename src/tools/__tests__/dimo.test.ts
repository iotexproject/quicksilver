...
describe('DimoTool', () => {
  ...
  describe('constructor', () => {
    it('should throw error if environment variables are missing', () => {
      process.env = {};
      expect(() => new DimoTool()).toThrow('Missing one of the following environment variables');
    });

    it('should initialize with correct properties', () => {
      expect(dimoTool.name).toBe('DIMO');
      expect(dimoTool.description).toContain('Tool for interacting with personal vehicles data and signals');
  ...
  describe('getRawData', () => {
    ...
    it('should handle network errors', async () => {
      vi.spyOn(dimoTool['dimo'].telemetry, 'query').mockRejectedValueOnce(new Error('Network error'));
