  ...
  describe("getRawData", () => {
    ...
    it("should handle successful API response", async () => {
      ...
      expect(axios.get).toHaveBeenCalledWith(
        `${baseUrl}?country=us&apiKey=${mockApiKey}`
      );
    });
    ...
    it("should handle successful API response with category", async () => {
      ...
      expect(result).toEqual(mockNewsData);
      expect(axios.get).toHaveBeenCalledWith(
        `${baseUrl}?country=us&apiKey=${mockApiKey}&category=technology`
      );
    });
    ...
    it("should handle successful API response with search query", async () => {
      ...
      expect(result).toEqual(mockNewsData);
      expect(axios.get).toHaveBeenCalledWith(
        `${baseUrl}?country=us&apiKey=${mockApiKey}&q=test+search`
      );
    });

    it("should throw error for invalid category", async () => {
      await expect(
        tool.getRawData({ category: "invalid-category" })
