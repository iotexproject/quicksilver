...
describe('DePINMetricsTool', () => {
  ...
  it('should initialize with correct properties', () => {
    ...
    expect(metricsTool.description).toBe('Fetches Global DePINScan metrics for market analysis');
  ...
  describe('getRawData', () => {
    it('should fetch and validate metrics data', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(expect.stringContaining(DEPIN_METRICS_URL + '?is_latest=true'));
    });
    ...
    it('should handle API errors', async () => {
      ...
      await expect(metricsTool.getRawData({})).rejects.toThrow('API request failed with status: 404');
    });
  ...
  describe('execute', () => {
    ...
    it('should handle latest metrics without volume field', async () => {
      ...
      const result = await metricsTool.schema[0].tool.execute({ isLatest: true }, executionOptions);
    ...
    it.skip('should handle real data (only latest metrics)', async () => {
      ...
      await metricsTool.schema[0].tool.execute({ isLatest: true }, executionOptions);
    });
    it.skip('should handle real data (only latest metrics)', async () => {
      const originalFetch = global.fetch;
      vi.unstubAllGlobals();
      ...
      await metricsTool.schema[0].tool.execute({ isLatest: false }, executionOptions);
    });
    it('should handle both string and number formats in metrics', async () => {
      ...
      const result = await metricsTool.schema[0].tool.execute({ isLatest: false }, executionOptions);
...
describe('DePINProjectsTool', () => {
  ...
  describe('getRawData', () => {
    ...
    it('should handle API errors', async () => {
      ...
      await expect(projectsTool.getRawData()).rejects.toThrow('API request failed with status: 404');
    });
  ...
  describe('execute', () => {
    ...
    it('should filter projects by category case-insensitively', async () => {
      const result = await projectsTool.schema[0].tool.execute({ category: 'Chain' }, executionOptions);
      expect(result.totalProjects).toBe(2);
      expect(result.projects.map(p => p.name)).toEqual(['Solana', 'IoTeX']);
    });

    it('should filter projects by minimum market cap', async () => {
      const result = await projectsTool.schema[0].tool.execute({ minMarketCap: 400000000 }, executionOptions);
      expect(result.totalProjects).toBe(1);
      expect(result.projects.map(p => p.name)).toEqual(['Solana']);
    });

    it('should filter projects by minimum devices', async () => {
      const result = await projectsTool.schema[0].tool.execute({ minDevices: 1000 }, executionOptions);
      expect(result.totalProjects).toBe(2);
      expect(result.projects.map(p => p.name)).toEqual(['Filecoin', 'IoTeX']);
    });

    it('should filter projects by layer1', async () => {
      const result = await projectsTool.schema[0].tool.execute({ layer1: 'Solana' }, executionOptions);
      expect(result.totalProjects).toBe(1);
    ...
    it('should handle null values in number formatting', async () => {
      const filecoin = (await projectsTool.schema[0].tool.execute({}, executionOptions)).projects.find(
        p => p.name === 'Filecoin'
      );
    ...
    it('should handle empty string values in number formatting', async () => {
      const solana = (await projectsTool.schema[0].tool.execute({}, executionOptions)).projects.find(
        p => p.name === 'Solana'
      );
    ...
    it('should filter out projects with undefined token when requireToken is true', async () => {
      ...
      const result = await projectsTool.schema[0].tool.execute({ requireToken: true }, executionOptions);
    ...
    it('should filter projects by minimum token price', async () => {
      const result = await projectsTool.schema[0].tool.execute({ minTokenPrice: 1.0 }, executionOptions);
    ...
    it('should filter projects by minimum estimated daily earnings', async () => {
      const result = await projectsTool.schema[0].tool.execute({ minDailyEarnings: 1.5 }, executionOptions);
    ...
    it('should filter projects by maximum days to breakeven', async () => {
      const result = await projectsTool.schema[0].tool.execute({ maxDaysToBreakeven: 50 }, executionOptions);
    ...
    it('should exclude descriptions when requireDescription is false', async () => {
      const result = await projectsTool.schema[0].tool.execute({ requireDescription: false }, executionOptions);
    ...
    it('should include descriptions when requireDescription is true', async () => {
      const result = await projectsTool.schema[0].tool.execute({ requireDescription: true }, executionOptions);

      expect(result.totalProjects).toBe(3);
      expect(result.projects[0].description).toBe('Solana is a general purpose layer 1 blockchain...');
      expect(result.projects[1].description).toBe('Filecoin is a peer-to-peer network...');
      expect(result.projects[2].description).toBe('IoTeX network...');
    });

    it('should exclude descriptions by default', async () => {
      const result = await projectsTool.schema[0].tool.execute({}, executionOptions);
    ...
    describe('numeric filters', () => {
      ...
      it('should filter out zero and null/undefined values for minDailyEarnings', async () => {
        const result = await projectsTool.schema[0].tool.execute({ minDailyEarnings: 0 }, executionOptions);
      ...
      it('should filter out zero and null/undefined values for minMarketCap', async () => {
        const result = await projectsTool.schema[0].tool.execute({ minMarketCap: 0 }, executionOptions);
      ...
      it('should filter out zero and null/undefined values for minTokenPrice', async () => {
        const result = await projectsTool.schema[0].tool.execute({ minTokenPrice: 0 }, executionOptions);
      ...
      it('should filter out zero and null/undefined values for minDevices', async () => {
        const result = await projectsTool.schema[0].tool.execute({ minDevices: 0 }, executionOptions);
      ...
      it('should filter out zero and null/undefined values for maxDaysToBreakeven', async () => {
        const result = await projectsTool.schema[0].tool.execute({ maxDaysToBreakeven: 100 }, executionOptions);
