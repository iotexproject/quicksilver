...
const NubilaCoordinatesSchema = z.object({
  lat: z.number().min(-90).max(90).describe('Latitude coordinate between -90 and 90 degrees'),
  lon: z.number().min(-180).max(180).describe('Longitude coordinate between -180 and 180 degrees'),
});
...
abstract class BaseWeatherAPITool extends APITool<CoordinatesInput> {
  ...
  public async getRawData(coords: CoordinatesInput): Promise<WeatherData | WeatherForecast> {
...
export class CurrentWeatherAPITool extends BaseWeatherAPITool {
  ...
  constructor() {
    super(CurrentWeatherToolSchema.name, CurrentWeatherToolSchema.description, 'weather');
  }
}
...
export class ForecastWeatherAPITool extends BaseWeatherAPITool {
  ...
  constructor() {
    super(ForecastWeatherToolSchema.name, ForecastWeatherToolSchema.description, 'forecast');
  }
}
